...
The thoughtful naming of these variables is a key component of clean code. If the above code just said `x = 28{:python}` the reader would have no reasonable way of ascertaining that `x` refers to an age. If the code instead said `age_of_the_person_called_alexis = 28{:python}` thta may be quite specific and clear to the reader, but would add unnecessary complexity!
